...
public class MainScreen extends BaseScreen {
    ...
    protected void init() {
        ...
        UIHelper.createButtonRow(this, buttonsY, button_count, x -> {
            ...
            switch (index) {
                ...
                case 3 -> this.pinButton = this.addRenderableWidget(new DarkButtonWidget(x, buttonsY, UIHelper.BUTTON_WIDTH, UIHelper.BUTTON_HEIGHT,
                        Component.translatable("gui.buildnotes.pin_button"), b -> pinSelected()));
    ...
    private void updateActionButtons() {
        ...
        if (currentTab == TabType.NOTES) {
            ...
            if (hasSelection) {
                ...
                this.pinButton.setMessage(Component.translatable(isPinned ? "gui.buildnotes.unpin_button" : "gui.buildnotes.pin_button"));
            }
        } else { // BUILDS tab
    ...
    private void pinSelected() {
        if (currentTab == TabType.NOTES) {
            NoteListWidget.NoteEntry sel = noteListWidget.getSelected();
            if (sel != null) {
                DataManager.getInstance().togglePinNote(sel.getNote().getId());
                refreshData(); // Re-sorts the list and refreshes UI
            }
        } else {
            BuildListWidget.BuildEntry sel = buildListWidget.getSelected();
            if (sel != null) {
